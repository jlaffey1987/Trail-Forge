...
- **trailforge** (`artifacts/trailforge`) — Off-road navigator web app (React + Vite, Supabase, Mapbox/Leaflet).
  ...
  - **AI grading + external discovery (migration 0007)**: trails carry `source` (`user` / `tet` / `act` / `ai-forum` / `ai-approx`), `source_url`, `verification_status` (`verified` / `ai-approximated` / `unverified`), `ai_grade`, `ai_grade_rationale`, `ai_grade_model`, `ai_graded_at`. Admin-only endpoints under `/api/admin/*` (forum scan, harvest TET/ACT, review queue, grade backfill) are gated by the `system_admins` table — bootstrap with `SYSTEM_ADMIN_USER_IDS` env var on the API server. Admin dashboard lives at `/admin`. The AI tab calls `POST /api/ai/chat` with the current map bbox to ground replies in nearby and saved trails. Re-grade button on trail detail calls `POST /api/trails/:id/grade-ai`. TET/ACT licensing decision (link-out only — no GPX rehost) is documented in `artifacts/trailforge/docs/tet-act-licensing-spike.md`.
