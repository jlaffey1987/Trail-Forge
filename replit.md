...
- **trailforge** (`artifacts/trailforge`) — Off-road navigator web app (React + Vite, Supabase, Mapbox/Leaflet).
  ...
  - **Google Maps-style navigation**: The NavigationView uses a dark road-map base layer (CARTO Dark Matter) instead of satellite imagery, with heading-up map rotation, a top-down motorbike SVG marker, and a compass widget. Key modules: `src/lib/useHeading.ts` (device orientation + GPS heading with heavy low-pass smoothing α=0.05, 3° dead zone, and render-gated setState to avoid per-frame re-renders), `src/lib/navigationReroute.ts` (off-route detection, auto re-route logic for road sections via OSRM with 10s cooldown and 3-failure give-up; trail sections never auto-reroute). The map container is oversized (130%) and CSS-rotated for heading-up mode; the compass widget in the corner toggles heading-up / north-up (persisted in localStorage). Off-route on trail sections shows "Return to the marked path. Trail sections aren't recalculated." Off-route on road sections triggers automatic OSRM re-routing with "Recalculating…" → "Re-routed" toasts. **Road-only navigation**: users can build navigation with just start/end addresses — no trails required. "Build Nav" button (blue) sits beside "Find Trails" in the planner controls, plus a second "Build Navigation" button at the bottom of the search results list. Both call `handlePlanTrip` and are enabled when start+end addresses are filled. **Nearby trails on navigation map**: after route assembly, trails within 3 km of the route corridor are fetched and rendered as faint dashed orange polylines (opacity 0.25) on the NavigationView map. Tapping shows trail name/distance/difficulty in a popup. Uses `getTrailLatLngs` + `distancePointToPolylineM` for corridor filtering; race-protected with a fetch counter ref.
