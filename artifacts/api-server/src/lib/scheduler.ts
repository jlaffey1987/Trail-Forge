...
import { runForumScan, runGradeBackfill } from "../routes/ai";
...
export function startAiScheduler(): void {
  if (!ENABLED) {
    logger.info(
      "AI scheduler disabled (set AI_SCAN_ENABLED=true to enable). Forum scan + grade backfill remain available via /api/admin/* endpoints.",
    );
    return;
  }
  if (scanTimer || backfillTimer) {
